...
func TestRange(t *testing.T) {
	...
	goodValidators := []Range{{Min{10}, Max{100}}, ValidRange(10, 100)}
	...
	goodValidators = []Range{{Min{10}, Max{10}}, ValidRange(10, 10)}
	for _, rangeValidator := range goodValidators {
		performTests(rangeValidator, tests, t)
	}
	...
	for i, num := range []int{50, 100, 10, 9, 101, 0, -1} {
		tests[i] = Expect{num, false, "min > val < max"}
	}
	...
	badValidators := []Range{{Min{100}, Max{10}}, ValidRange(100, 10)}
